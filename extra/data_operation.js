...
const spiData = [
  ...
  //spi more than 5
  app.get("/spiData/more/:spi",(req,res) => {
    const spi = req.params.spi;
    const data = spiData.filter((temp) => {
      if(Number(temp.spi > spi)){
       return true;
      }
    });
    const ans = {
...
//spi less than 5
app.get("/spiData/less/:spi",(req,res) => {
  const spi = req.params.spi;
  const data = spiData.filter((temp) => {
    if(Number(temp.spi < spi)){
     return true;
...
//print all record with Hindi language
app.get("/spiData/Language/Hindi",(req,res) => {
//const language_name = req.params.name;
  const data = spiData.filter((temp) => {
    if(temp.name == "Hindi"){
     return true;
    }
...
//print all record with Sindhi language and name starts with A
app.get("/spiData/Language/Sindhi/:name",(req,res) => {
//const language_name = req.params.name;
  const data = spiData.filter((temp) => {
    if(temp.language == "Sindhi" && temp.name.startsWith(req.params.name)){
     return true;
    }
  });
  const ans = {
